                    ...
                    <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-2 sm:gap-0">
                        <div class="grid grid-cols-2 w-full sm:w-1/2">
                            ...
                            <div>
                                ...
                                <p class="">€{{ number_format($order->orderItems->sum(function($item) {
                        ...
                        <div class="flex flex-row sm:flex-col gap-2 w-full sm:w-1/3">
                            @foreach ($order->orderItems as $item)
                                <button class="w-1/2 sm:w-full h-10 gap-2 flex items-center justify-center text-center bg-[#DDA15E] hover:bg-[#BC6C25] text-[#FEFAE0] py-3 rounded-lg transition-all ease-in-out">
                                    <i class="fa-solid fa-circle-arrow-left"></i> <a href="{{ route('store.show', $item->product->id) }}">Bekijk Item</a>
                                </button>
                            @endforeach
                            
                            <form method="POST" action="{{ route('cart.add', $item->product) }}" class="w-1/2 sm:w-full">
                                @csrf
